...
pub unsafe extern "C" fn SQLColumnsW(
    ...
    panic_safe_exec_clear_diagnostics!(
        ...
        || {
            ...
            let catalog_string: String;
            let catalog = if catalog_name.is_null() {
                None
            } else {
                catalog_string = input_text_to_string_w(catalog_name, catalog_name_length.into());
                if catalog_string.is_empty() {
                    None
                } else {
            ...
            let table_string: String;
            let table = if table_name.is_null() {
                None
            } else {
                table_string = input_text_to_string_w(table_name, table_name_length.into());
                Some(table_string.as_str())
            };
            //let column_name_string = input_text_to_string_w(column_name, column_name_length.into());
            let column_name_string: String;
            let column = if column_name.is_null() {
                None
            } else {
                column_name_string = input_text_to_string_w(column_name, column_name_length.into());
    ...
    let sql_return = match FromPrimitive::from_u16(info_type) {
        Some(some_info_type) => {
            ...
            match some_info_type {
                ...
                InfoType::SQL_KEYWORDS => SqlReturn::SUCCESS,
                _ => {
